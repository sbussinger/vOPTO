...
bool localFile::LockFile(Bit8u mode, Bit32u pos, Bit32u size)
	...
	int curpos = _tell(fd);									// _locking starts with current position so save and set
	if (curpos != -1 && _lseeki64(fd, pos, SEEK_SET) != -1)	// no positioning  for this file!
		{													// _lseeki64() cause pos is unsigned here and _lseek() uses 32 bit signed!
		if (!_locking(fd, mode == 0 ? _LK_NBLCK : _LK_UNLCK, size))
			{
			_lseek(fd, curpos, SEEK_SET);					// restore original position !
			return true;
			}
